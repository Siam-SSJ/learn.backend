...
const generateAccessAndRefereshTokens = async (userId) => {
    try {
        ...
        throw new ApiError(500,"somethisng went wrong while generating refresh and access token")
    }
}
...
const loginUser = asyncHandler( async (req,res) => {
    ...
    const loggedInUser = await User.findById(user._id).select("-password -refrestToken")
    ...
    const channel = await User.aggregate([
        ...
        },
        {
            $addFields:{
                ...
                isSubscribed:{
                    $cond: {
                        if:{$in:[req.user?._id, "$subscribers.sub.subscriber"]},
                        then: true,
                        else: false
                    }
                }
